...
<div class="row">
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.phone name="phone" label="phone" :value="old('phone', $editing ? $user->phone : '')" maxlength="255" placeholder="phone"
            required></x-inputs.phone>
    </x-inputs.group>
    ...
    <x-inputs.group class="col-sm-12">
        <div x-data="imageViewer('{{ $editing && $user->image ? \Storage::url($user->image) : '' }}')">
    ...
    <div>
        <livewire:users.leaders-and-roles-selector :user="$user ?? null" :editing="$editing ?? false" />
    </div>
